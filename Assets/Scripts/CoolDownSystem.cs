...
public class CoolDownSystem : MonoBehaviour
{
    ...
    public float GetOverallDuration(int Id)
    {
        foreach (CoolDownData cooldown in coolDowns)
        {
            if (cooldown.Id != Id) continue;

            return cooldown.overallDuration;
        }

        return 0f;
    }

    public float GetNormilizedTime(int Id)
    {
        foreach (CoolDownData cooldown in coolDowns)
        {
            if (cooldown.Id != Id) continue;

            return  cooldown.RemainingTime / cooldown.overallDuration;
        }
